  ...
  async sendRequest(senderId: string, receiverId: string) {
    ...
    if (receiver.fcmToken) {
      ...
      this.notificationService.sendAndSave(
        receiver.id,
        'New Request',
        `Patient ${patientName} send a request`,
        'new_request',
        { requestId: newRequest.id, screen: 'DoctorHome' }
    ...
    if (newStatus === 'accepted') {
      return await db.transaction(async (tx) => {
        ...
        //   where(and(ne(requests.id, requestId), eq(requests.senderId, updatedRequest.senderId)))
          await tx.update(requests).set({status:'delivered'}).
          where(and(ne(requests.id, requestId), eq(requests.senderId, updatedRequest.senderId)))
        ...
        let patient = await db.select().from(users).where(eq(users.fusionAuthId, conversation.patientId))
        let doctor = await db.select().from(users).where(eq(users.fusionAuthId, conversation.patientId))
        if (patient[0].fcmToken) {
          await this.notificationService.sendAndSave(
            patient[0].fusionAuthId,
            accepted ? ` doctor ${doctor[0].firstName} accept your request` : ` doctor ${doctor[0].firstName} reject your request`,
            accepted ? ` your request is accepted by ${doctor[0].firstName} ` : ` your last request ${doctor[0].firstName}`,
            'accept request',
            {
              screen: 'Cases'
            }
