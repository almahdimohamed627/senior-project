...
export class DiagnosesPdfService {
  ...
  async generateDiagnosisPdf(conversationId: number): Promise<string> {
    ...
    <html>
      <head>
        ...
        <style>
          ...
          img { max-width: 350px; margin-top: 10px; border: 1px solid #ddd; }
    ...
    // 5. تشغيل Puppeteer (تم التعديل ليعمل على السيرفر واللوكال)
    console.log(process.env.CHROMIUM_PATH)
    const browser = await puppeteer.launch({
      headless: true,
      // يستخدم المتغير البيئي إذا وجد، وإلا يستخدم المسار الافتراضي للينكس
      // ملاحظة: على جهازك المحلي (Windows/Mac) يفضل وضع مسار الكروم في ملف .env
    ...
    // إرجاع المسار الكامل للملف ليتم تخزينه في الداتابيز
    return pdfPath;
  }
  ...
  async getPdfPath(conversationId: number): Promise<string> {
    ...
    // تصحيح المسار:
    // إذا كان المسار المخزن في الداتابيز كاملاً (Absolute)، نستخدمه كما هو.
    // إذا كان مجرد اسم ملف، نقوم بدمجه مع مجلد uploads.
    let fullPath = path.isAbsolute(dbPath) 
        ? dbPath 
        : path.join(process.cwd(), 'uploads', dbPath);

    console.log('Checking PDF Path:', fullPath);
