def build() {
    echo "🏗️ Starting build for backend component"
        ...
        withCredentials([file(credentialsId: credentialId, variable: 'ENV_FILE')]) {
            ...
            stage('Clean Previous Builds') {
                sh '''
                    ...
                    docker rmi --force backend-nest-app
            ...
            stage('Build Services') {
                ...
                if (shouldBuild) {
                    sh '''
                        ...
                        docker build --no-cache --pull -t backend-nest-app .

                        # Even with direct build, try a final compose build to ensure service configuration is updated
                        docker compose --env-file .env build --no-cache nest-app || echo "Compose build may be redundant but attempted."
                    '''
                } else {
                    echo "⚠️ No build configuration found - skipping build"
                }
            }
...
def deploy() {
    echo "🚀 Starting deployment for backend component"
        ...
        sh '''
            ...
            if ! curl -s -f http://localhost:3000/health >/dev/null 2>&1 && 
               ! curl -s -f http://localhost:3000 >/dev/null 2>&1; then
                ...
                docker compose --env-file .env logs --tail=50 nest-app || true
            fi
        '''

        echo "✅ backend deployment completed successfully"
    } catch (Exception e) {
        echo "❌ Deployment failed for backend: ${e.message}"
        sh """
            echo "=== Debug Information ==="
            docker compose --env-file .env ps 2>/dev/null || true
            echo "=== Recent Logs ==="
...
def cleanup() {
    ...
    sh '''
        docker compose down --remove-orphans --rmi all 2>/dev/null || true
        docker rmi -f backend-nest-app 2>/dev/null || true
    '''
}
