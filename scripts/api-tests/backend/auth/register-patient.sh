...
if validate_registration "$REGISTER_RESPONSE" "patient"; then
    ...
    echo "$RESULT_JSON"
    echo "✅ Patient registration test passed"
    exit 0
else
    END_TIME=$(date +%s)
    DURATION=$((END_TIME - START_TIME))
    ...
    log_test_result "$RESULT_JSON"
    echo "$RESULT_JSON"
    echo "❌ Patient registration test failed"
    exit 1
fi